...
class User(AbstractUser):
    ...
    unit = models.ForeignKey('Unit', on_delete=models.CASCADE, related_name='users', blank=True, null=True)
...
class Unit(models.Model):
    ...
    duty_positions = models.PositiveIntegerField(validators=[MinValueValidator(1), MaxValueValidator(3)])
    owner = models.OneToOneField('User', on_delete=models.CASCADE, related_name='owned_unit')
    # doctors (Doctor)
...
class Doctor(models.Model):
    ...
    unit = models.ForeignKey('Unit', on_delete=models.CASCADE, related_name='doctors')
    impersonated_user = models.OneToOneField(
        'User', related_name='my_doctor', on_delete=models.CASCADE, blank=True, null=True
    )
    owner = models.ForeignKey('User', on_delete=models.CASCADE, related_name='owned_doctors')
...
class DoctorMonthlyData(models.Model):
    ...
    owner = models.ForeignKey('User', on_delete=models.CASCADE, related_name='owned_doctors_monthly_data')
...
class MonthlyDuties(models.Model):
    ...
    number_of_days = models.PositiveIntegerField(validators=[MaxValueValidator(31)])
    first_weekday = models.PositiveIntegerField(validators=[MinValueValidator(0), MaxValueValidator(6)])
    duty_positions = models.PositiveIntegerField(validators=[MinValueValidator(1), MaxValueValidator(3)])
    unit = models.ForeignKey('Unit', on_delete=models.CASCADE, related_name='monthly_duties')
    owner = models.ForeignKey('User', on_delete=models.CASCADE, related_name='owned_monthly_duties')
